...
function scoreMatch(query: string, item: FaqItem) {
  ...
  // simple scoring: substring match + keyword overlap
  let score = 0;
...
export default function SupportBot() {
        ...
        setMessages([
          {
            id: "m0",
            from: "bot",
            text: "Hi 👋 I’m the ADW helper. Ask a question or pick a topic below.",
          },
        ]);
      }
    } catch {
  ...
  function answerFromFaq(userText: string) {
    ...
    if (!top || top.s < 2) {
      return {
        text:
          "I can help with booking, bidding, payments, driver onboarding, and payouts. Try a short question like “How do I book?” or pick a topic below.",
        links: [
          { label: "How it works", href: "/how-it-works" },
  ...
  function send(text: string) {
    ...
    setTimeout(() => {
      setMessages((m) => {
        const next = [...m, botMsg];
        // Attach links as a follow-up bot message if any
  ...
  function reset() {
    setQuery("");
    setMessages([
      { id: "m0", from: "bot", text: "Hi 👋 I’m the ADW helper. Ask a question or pick a topic below." },
    ]);
    try {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50">
      ...
      {!open && (
        ...
        >
          <span className="flex h-9 w-9 items-center justify-center rounded-xl border border-white/15 bg-white/5 text-white/85">
            ?
          </span>
        ...
        <div className="w-[340px] overflow-hidden rounded-3xl border border-white/15 bg-[#0b0f17]/95 shadow-2xl backdrop-blur">
          ...
          <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
            <div className="flex items-center gap-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl border border-white/15 bg-white/5 text-white/85">
                ?
              </div>
              <div>
          ...
          <div className="border-t border-white/10 bg-white/[0.02] px-4 py-3">
            ...
            <div className="mt-2 text-[11px] leading-5 text-white/55">
              If you need help urgently, use the pages above to get started quickly.
            </div>
          </div>
        </div>
      )}
